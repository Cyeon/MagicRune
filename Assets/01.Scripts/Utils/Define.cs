...
public class Define
{
    ...
    public const string BENEFIC_DESC = "<color=#F9B41F>이로운 효과</color> 를 부여";
    public const string INJURIOUS_DESC = "플레이어한테 <color=#F9B41F>해로운 효과</color> 를 부여";
    public static string DamageDesc(int damage, int count = 1)
    {
        if(count > 1)
        {
            return "<color=#369AC2>" + damage + "</color> 대미지로 <color=#369AC2>" + count + "</color> 번 <color=#F9B41F>공격</color>";
        }

        return "<color=#369AC2>" + damage + "</color> 대미지로 <color=#F9B41F>공격</color>";
    }
}