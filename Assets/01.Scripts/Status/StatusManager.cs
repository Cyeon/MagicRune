...
{
    public List<Status> statusList = new List<Status>(); // Î™®Îì† ?ÅÌÉú?¥ÏÉÅ Î™©Î°ù
        ...
        if (unit.IsDie == false)
        {
            ...
            if (unit.unitStatusDic.TryGetValue(status.invokeTime, out statusList))
            {
                ...
                if (currentStauts != null)
                {
                    ...
                    //unit.unitStatusDic[status.invokeTime].Remove(status); // ¡Ÿ¿Ã∞Ì
                    //_dialScene?.RemoveStatusPanel(unit, status.statusName); // ∏∏æ‡ 0¿Ã«œ∂Û∏È ¡ˆøÏ±‚ // æ∆¥œ∏È æ˜µ•¿Ã∆Æ
                }
                else
                {
                    Debug.LogWarning(string.Format("{0} status is not found. Can't Remove do it.", status.statusName));
