...
public class RestDialElement : MonoBehaviour
{
    ...
    private List<RestRuneUI> _runeList;
    private RestRuneUI _selectCard;
    public RestRuneUI SelectCard
    {
        get => _selectCard;
        set
        {
            ...
            if (transform.eulerAngles.z <= _dial.RuneAngle / 2 || transform.eulerAngles.z >= (360f - _dial.RuneAngle / 2))
            {
                ...
                if (inBoolean)
                {
                    ...
                    if (_isRotate == true)
                    {
                        if (_selectCard != null)
                        {
                            _dial.EditText(_selectCard.Desc);
                        }
                    }
                }
                else if (outBoolean)
    ...
    public void MoveRune(int index, bool isLeft = true)
    {
        if (isLeft == true)
        {
            RestRuneUI rune = _runeList[index];
    ...
    public void AddRuneList(RestRuneUI rune)
    {
        _runeList.Add(rune);
    }
    ...
    public void SetRuneList(List<RestRuneUI> list)
    {
        _runeList = new List<RestRuneUI>(list);
    }
