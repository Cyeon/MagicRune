...
Shader "Lite Effect/Particles/Lite Particle Blend"
{
	Properties
	{
		...
		SubShader
		{
			...
			Pass
			{
				...
				half4 frag(v2f i) : SV_Target
				{
					...
					float4 ca = lerp(float4(1,1,1,1) ,float4(1,1,1, tex2D(_CutTex, i.uv_CutOut).a),_isCut);
					c *= ca;
					...
					c.rgb += Light * 0.2 +  Light * i.color.rgb ;
					c.a += Light;
